...
class FDataBase:
    ...
    def add_post(self, title, article, url):
        try:
            self.__cur.execute(f'SELECT COUNT() as "count" FROM posts '
                               f'WHERE url LIKE "{url}"')
            ...
            article = re.sub(r"(?P<tag><img\s+[^>]*src=)(?P<quote>[\"'])"
                             r"(?P<url>.+?)(?P=quote)>",
                             "\\g<tag>" + base + "/\\g<url>>", article)

            tm = math.floor(time.time())
            self.__cur.execute('INSERT INTO posts VALUES(NULL, ?, ?, ?, ?)',
                               (title, article, tm, url))
            self.__db.commit()
    ...
    def get_post(self, alias):
        try:
            self.__cur.execute(f'SELECT title, article FROM posts '
                               f'WHERE url LIKE "{alias}" LIMIT 1')
            res = self.__cur.fetchone()
            if res:
                return res
        except sqlite3.Error as e:
    ...
    def get_posts_announce(self):
        try:
            self.__cur.execute(f'SELECT id, title, article, url '
                               f'FROM posts ORDER BY created_time DESC')
            res = self.__cur.fetchall()
            if res:
                return res
        except sqlite3.Error as e:
    ...
    def add_user(self, name, email, hpwd):
        try:
            self.__cur.execute(f'SELECT COUNT() as "count" '
                               f'FROM users WHERE email LIKE "{email}"')
            res = self.__cur.fetchone()
            if res['count'] > 0:
